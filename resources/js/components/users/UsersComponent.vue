    ...
    <div class="my-3">
        ...
        <div class="form-group">
          <div class="input-group mb-1">
            ...
            <button v-on:click="clearSearchItem" :class="{'disabled': searchItem==''}"
                    ...
                    <button class="btn btn-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="bi bi-funnel-fill"></i></button>
                    <ul class="dropdown-menu dropdown-menu-end">
                      <li><a @click="searchInTheList('Новый пользователь')" class="dropdown-item" href="#">Новые</a></li>
                      <li><a @click="searchInTheList('Онлайн')" class="dropdown-item" href="#">Онлайн</a></li>
                      <li><a @click="searchInTheList('Офлайн')" class="dropdown-item" href="#">Офлайн</a></li>
                      <li><hr class="dropdown-divider"></li>
                      <li><a @click="searchInTheList('Администратор')" class="dropdown-item" href="#">Администраторы</a></li>
                      <li><a @click="searchInTheList('Менеджер')" class="dropdown-item" href="#">Менеджеры</a></li>
                      <li><a @click="searchInTheList('Собственник')" class="dropdown-item" href="#">Собственники</a></li>
                    </ul>
          </div>
        </div>
        <Loader v-if="loader"/>
            ...
            <tbody>
              <tr v-for="item in paginatedItems" class="bg-info">
                ...
                <td class="d-flex justify-content-between align-items-center rounded-end">
                  <button class="btn btn-light" type="button" @click="showProfile(item.pId, 'edit')">
                    <i class="bi bi-pencil-square"></i>
                  </button>
                  <button class="btn btn-light" type="button" @click="showProfile(item.pId, 'show')">
      ...
      <div v-else-if="edit">
        <div class="d-grid gap-2 mb-2">
          <button class="btn btn-dark"  @click="showProfile(null)" type="button">Назад</button>
        </div>
        <profile-edit-component :profile-id="id" :key="id"/>
      </div>
      <div v-else-if="show">
        <div class="d-grid gap-2 mb-2" >
          <button class="btn btn-dark" @click="showProfile(null)" type="button">Назад</button>
...
<script>
    export default {
        ...
        methods: {
          getUsers(){
            this.loader = true
            axios.post('/api/get-users').then((response) =>{
          ...
          showProfile(id, type){
            if(type === 'edit'){
              this.edit = true
              this.show = this.search = false
            }else if(type=== 'show'){
              this.show = true
              this.edit = this.search = false
            }else{
              this.search = true
              this.edit = this.show = false
            }
            this.id = id
          },
          reloadList(){
