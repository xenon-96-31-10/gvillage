<template>
    <div>
      <div class="alert alert-secondary alert-dismissible fade show" role="alert">
        ...
        <h4 class="alert-heading">Добро пожаловать! Это управление объектами, которые Вам доступны!</h4>
        <p>
          Вы можете просмотривать и редактировать информацию об объектах.
        </p>
        <hr>
      ...
      <div v-else>
        ...
        <div v-else-if="create">
          <div class="d-grid gap-2 mb-2">
            ...
            <project-create-component @refreshList="refreshList"/>
          </div>
        </div>
        <div v-else-if="show">
          <div class="d-grid gap-2 mb-2">
          ...
          <project-show-component :project-id="id" :key="id"/>
        </div>
        <div v-else-if="edit">
          <div class="d-grid gap-2 mb-2" >
            <button class="btn btn-dark" @click="btnProject(null)" type="button">Назад</button>
          </div>
          <project-edit-component :project-id="id" :key="id"/>
        </div>
      </div>
...
<script>
    ...
    export default {
        ...
        methods: {
          getProjects(){
            this.loader = true
            axios.post('/api/get-projects-for-list').then((response) =>{
